...
- **State**: `likedRestaurants` (Metadata), `swipedIds` (Set for lookup), `pendingSync` (Offline queue).
- **Actions**: `addSwipe`, `removeLikedRestaurant`, `hydrateStore`, `clearStore`.
- **Persistence**: Using `AsyncStorage` via `persist` middleware with custom `Set` rehydration logic.
- **Strategy**: 60fps local updates with background reconciliation via `useSync` hook.

### Firebase Schema
- **Collection: `users`**
    - `uid`: Unique identifier from Firebase Auth (Anonymous/Linked).
    - `interactions` (Sub-collection): Tracks every swipe (`like`/`dislike`) to prevent display repetition.
    - `matches` (Sub-collection): Stores denormalized restaurant metadata to minimize API latency and costs.
- **Indexing**: Optimized for temporal queries using `timestamp` and `syncedAt` fields.
...
| Scenario | Current Handling | Implementation Status |
| :--- | :--- | :--- |
| **No Internet** | Generic Axios error catch + `pendingSync` queue. | **REVOLVED** (Queue-based Sync) |
| **Location Denied** | Fallback to Mock Data (Filtered). | Pending enhancement. |
| **API Limit Reached** | Error state with log. | Planned: Firestore Caching. |
| **Empty Results** | Recursive fetch with `nextPageToken`. | **REVOLVED** (Recursive Loop) |
| **Firestore Failures** | Local-only (Zustand will re-sync via `useSync`). | **REVOLVED** (Reconciliation) |
| **GDPR Compliance** | `purgeUserData` service implemented. | **REVOLVED** (Account Purge) |
| **Stale Media** | Photo refresh logic in `imageService.ts`. | **REVOLVED** (Refresh Hook) |
| **New Device** | `hydrateStore()` triggered on auth changes. | **REVOLVED** (Cloud Hydration) |
...
- **Technical Debt**:
    - [ ] `useRestaurants` needs more robust error boundary implementation for API failures.
    - [ ] Implement Bloom-filter for server-side interaction filtering to reduce payload size.
    - [ ] Filter logic UI modal for `DiscoverScreen` (Planned next).
